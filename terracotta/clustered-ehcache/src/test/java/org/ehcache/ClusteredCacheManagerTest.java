...
public class ClusteredCacheManagerTest {
  ...
  public void testNothingButAPI() {
    ...
    // Pass TC Config in, and retrieve a ClusteredCacheManager to get access to live cycle stuff &
    // server-side stuff (e.g. config)
    final ClusteredCacheManager cacheManager = CacheManagerBuilder.newCacheManagerBuilder()
        .with(tcConfig).build(true);

    // UnclusteredCache
    final Cache<String, Object> foo = cacheManager.createCache("foo",
        new BaseCacheConfiguration<String, Object>(String.class, Object.class, (EvictionVeto) null, // Whatever
                                                                                                    // this
                                                                                                    // is
            (EvictionPrioritizer) null, // Whatever this is
            // Whatever this is
            this.getClass().getClassLoader(), (Expiry) null, ResourcePoolsBuilder
                .newResourcePoolsBuilder().heap(500, EntryUnit.ENTRIES).build()));

    // Clustered with all specified config stuff
    final Cache<String, Object> baz = cacheManager.createCache(
        "baz",
        new TerracottaBaseCacheConfiguration<String, Object>(
            new BaseClusteredCacheSharedConfiguration<String, Object>(
                // added this line
                String.class, Object.class, (EvictionVeto) null, (EvictionPrioritizer) null,
                (Expiry) null, 2, MemoryUnit.TB, false, // 2 TB non-persistent, resourcePool aren't
                                                        // (yet?) "splittable"
                false, // not transactional
                BaseClusteredCacheSharedConfiguration.CacheLoaderWriter.NONE // no CacheLoaderWriter
            ), // and closes here
            this.getClass().getClassLoader(), ResourcePoolsBuilder.newResourcePoolsBuilder()
                .heap(500, EntryUnit.ENTRIES).build()));

    // Accessing an existing clustered cache (or with all defaults? but that could lead to unclear
    // races)
    final Cache<String, Object> simple = cacheManager.createCache("simple",
        new TerracottaBaseCacheConfiguration<String, Object>(String.class, Object.class, this
            .getClass().getClassLoader(), ResourcePoolsBuilder // Local tier(s) only ?!
            .newResourcePoolsBuilder().heap(500, EntryUnit.ENTRIES).build()));

    // Share runtime config change
    final CacheRuntimeConfiguration<String, Object> runtimeConfiguration = simple
        .getRuntimeConfiguration();
    final ClusteredCacheSharedRuntimeConfiguration<String, Object> serverSideRuntimeConfig = cacheManager
        .getClusteredConfig(simple);
